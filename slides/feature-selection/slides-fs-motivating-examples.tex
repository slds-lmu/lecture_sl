...
\end{vbframe}
  ...
  \lz
  Feature selection methods used in practice are often (complex) combinations of these methods. Choice of best method depends on %data-generating process, 
  sample size, data structure, and the target of analysis.

 \lz
 
  \begin{itemize}
      \item Additionally, there are embedded methods that will not be covered in detail here.
  \end{itemize}

   %integrate feature selection during training.

  \end{vbframe}

  \begin{vbframe}{Embedded Methods}
  Embedded methods integrate feature selection directly into the optimization, using in-built criteria to control num. of selected features.\\
    %\begin{itemize}
      %\item Embedded techniques are methods that integrate feature selection directly into the learning process.
      %\item They use an internal optimization criterion to control the search for useful features.
    ...
    \begin{itemize}
      \item Regularizing the parameters of a model with an $L1$ penalty %in the empirical risk 
