...
p <- ggplot(df, aes(x=x, y=y)) + 
  geom_point(aes(color=DATA_COLOR), size = 3) + 
  geom_line(aes(y=prediction), color=MODEL_COLOR, size = 1.2) + 
  geom_rect(aes(
    xmin = bottom_left_x,
  ...
  scale_x_continuous(breaks = seq(-5, 5, by = 1)) + # x-axis increments of 1
  scale_y_continuous(breaks = seq(-5, 5, by = 1))  # y-axis increments of 1
