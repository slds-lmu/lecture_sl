...
\Lpiy = |\pi - y|
\end{equation*}
As the L1 loss is not a \textit{strictly} proper scoring rule (cf. section on proper scoring rules), it should not necessarily be used for calibration. %Note that it is not convex on probabilities anymore.
Despite convex in $\pi$, $$L(y,\pi(f))=|(1+\exp{(-f)})^{-1}-y|$$ as a composite function is not convex in $f$ anymore (log. sigmoid for $\pi$).
\vspace{-0.2cm}
\begin{center}
