...
\begin{itemize}
    ...
    \item $f:\Xspace \rightarrow \mathbb{R}^g, f \in \Hspace$ is a model which maps a feature vector to a numerically encoded element of $\Yspace$ and $\Hspace$ is the hypothesis space, 
...
While the risk minimizer gives us the (theoretical) optimal solution, the \textbf{optimal constant model} (also: featureless predictor) gives us a 
baseline solution in experiments, when we want to compare to a simple model.
