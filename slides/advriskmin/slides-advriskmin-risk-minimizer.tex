...
\vspace{-0.3cm}
$$\riskef = \sumin \Lxyi\,\quad \text{where}$$
\vspace{-0.2cm}
{\small
\begin{itemize}\setlength\itemsep{0.5pt} 
    \item each observation $\left(\xi, \yi\right) \in  \Xspace \times \Yspace$ and $\Xspace, \Yspace$ are the feature and target spaces, respectively
    \item $f_{\Hspace}:\Xspace \rightarrow \mathbb{R}^g, f \in \Hspace$ is a model which maps a feature vector to a numerically encoded element of $\Yspace$ and $\Hspace$ is the hypothesis space {\small(if $\Hspace$ is omitted it is clear from the context)} 
    \item $L:\left(\Yspace\times\mathbb{R}^g\right)\rightarrow\mathbb{R}$ is the loss function which measures the dissimilarity of the model prediction and the true target,
    \item and we assume that $\left(\xi, \yi\right)  \overset{\text{i.i.d.}}{\sim} \Pxy$ where $\Pxy$ is the distribution of the data generating process (DGP)
...
for a certain hypothesis $f \in \Hspace$ and a loss $L(y,f)$. 
...
While the risk minimizer gives us the (theoretical) optimal solution, the \textbf{optimal constant model} (also: featureless predictor) gives us a simple 
baseline solution in experiments.

\vspace{0.2cm}

By restricting $\Hspace$ to constant functions $f_c$ independenx of $\xv$, the constant model minimizing the theoretical risk $\E_{xy}\left[L(y,f(\xv)\right]$ is defined as:

$$f_{c}^{\ast} = \argmin_{c \in \mathbb{R}} \E_{xy} \left[L(y,c)\right] = \argmin_{c \in \mathbb{R}} \E_{y} \left[L(y,c)\right]$$

\vspace{0.3cm}

On the other hand, the constant model $\fx = \theta = c$ that optimizes the empirical risk $\risket$ is denoted as as $\hat{f}_c = \argmin_{c \in \mathbb{R}} \risket$.
