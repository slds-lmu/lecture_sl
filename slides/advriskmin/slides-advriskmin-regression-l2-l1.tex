...
\item For $L2$ loss, Bayes risk therefore reduces to 
\begin{align*}
    \risk(\fbayes(\xtil)) &= \Exy\left[(y-\fbayes(\xtil))^2\right] = \E_x \left[ \E_{y|x} \left[(y-\fbayes(\xtil))^2 \right] \right]\\
    &= \E_x \left[ \E_{y|x} \left[(y-\E_{y|x} \left[y ~|~ \xv = \xtil \right])^2 \right] \right] = \E_x \left[ \var(y | \xv = \xtil) \right]
\end{align*}
