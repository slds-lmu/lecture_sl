...
\newcommand{\learninggoals}{
  ...
  \item Statistical properties, robustness 
  \item Numerical properties
  \item Fundamental concepts like translation invariance
...
strongly affected by large residuals than others, if the go up superlinearly with the residuals (L2, not robust) or linearly (L1, 
 robust) -- or even sublinearly (really robust). 
...
\begin{minipage}[c]{0.4\textwidth}
  \small \raggedright
  $L2$ model is strongly influenced by outlier.
...
\begin{itemize}
 ...
 % \item A function is said to be $\mathcal{C}^k$ if it is $k$ times continuously differentiable. A function is $\mathcal{C}^\infty$ if it is continuously differentiable for all orders $k$. 
  ...
  \begin{itemize}
    \small
    \item If loss is unsmooth, we mighht have to use derivative-free optimization (or worse, in case of 0-1)
...
$$
  ...
  \end{flalign*}
\item Hence, problem unbounded from below
