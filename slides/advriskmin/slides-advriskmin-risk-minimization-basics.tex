...
\item To learn a model, we usually do ERM:
$$\riskef = \sumin \Lxyi$$
...
\item Minimizing theoretical risk, so expected loss over DGP, is major goal:
...
\begin{framei}[sep=L]{Hypothesis Spaces and Parametrization}

\item We often write $\riskf$, but finding an optimal $f$ is operationalized as finding optimal $\thetav \in \Theta$ among a family of parametrized curves:
...
\begin{framei}{Optimal loss values -- M-Estimators}
...
\imageC[0.32]{figure/l1_vs_l2.png}
...
\begin{align*}
	\fbayes_{\Hall} &= \argmin_{f \in \Hall} \risk(f) = \argmin_{f \in \Hall}\Exy\left[\Lxy\right]\\ &=  \argmin_{f \in \Hall}\int \Lxy \text{d}\Pxy
\end{align*}
...
\begin{framei}{Estimation and Approximation Error} 
...
\begin{framei}[sep=L]{(Universally) Consistent Learners \citelink{STNE1977CONS}}
% https://mlweb.loria.fr/book/en/consistency.html

\item \textbf{Consistency} is an asymptotic property of a learning algorithm, which ensures the algorithm returns \textbf{the correct model} when given \textbf{unlimited data}.

\item Let $\ind:\mathbb{D} \to \Hspace$ be a learning algorithm that takes a training set $\Dtrain \sim \Pxy$ of size $n_\text{train}$ and estimates a model $\hat f: \Xspace \to \R^g$. 

\item The learning method $\ind$ is said to be \textbf{consistent} w.r.t. a certain distribution $\Pxy$ if the risk of the estimated model $\hat f$ converges in probability ( \enquote{$\overset{p}{\longrightarrow}$}) to the Bayes risk $\riskbayes$ when $n_\text{train}$ goes to $\infty$: 
...
\item Consistency is defined w.r.t. a particular distribution $\Pxy$.
But since we usually don't know $\Pxy$, consistency
does not offer much help to choose an algorithm for a specific task. 

\item More interesting is the stronger concept of \textbf{universal consistency}: An algorithm is universally consistent if it is consistent for \textbf{any} distribution. 

\item In Stone's famous consistency theorem (1977), the universal consistency of a weighted average estimator such as KNN was proven. Many other ML models have since then been proven to be universally consistent (SVMs, ANNs, etc.).
