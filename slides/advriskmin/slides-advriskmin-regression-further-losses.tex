...
$$
\Lyf = \log \left( \cosh(\left|y - f\right|) \right)
...
\begin{figure}
      \centering
        \scalebox{0.99}{\includegraphics{slides/advriskmin/figure_man/logcosh-derivation.png}}
...
The log-cosh approach obtain a differentiable approximation of the $L1$ loss can also be extended to  differentiable pinball losses which are used, e.g., for quantile regression. For more details on the log-cosh loss, see  \citebutton{Saleh and Saleh, 2024}{https://arxiv.org/pdf/2208.04564}

\framebreak

\textbf{The cosh($\mu,\sigma$) distribution}:\,
The reciprocal $\text{cosh}(x)$ function further defines a distribution by its positivity on the whole real line and the fact that $\int_{-\infty}^{\infty} \frac{1}{\pi \text{cosh}(x)} \text{d}x = 1$.\\
\vspace{0.1cm}
Generally, we can define a location-scale family of distributions (parameterized by $\mu$ and $\sigma$) resembling a Gaussian with heavier tails:

$$f(x ; \mu, \sigma)=\frac{1}{\pi \sigma \cosh \left(\frac{x-\mu}{\sigma}\right)}, \quad \mathbb{E}[X]=\mu,\,\, \text{Var}[X]=\frac{\pi^2 \sigma^2}{4}$$

\begin{center}
\includegraphics[width = 0.72\textwidth]{figure_man/cosh-gaussian-densities.png}
\end{center}
