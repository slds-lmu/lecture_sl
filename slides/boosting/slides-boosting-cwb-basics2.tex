...
\item {\small Include an intercept BL}:
  \begin{itemize}
    ...
    \item {\footnotesize Final intercept is given as $\fm[0](\xv) + \hat{\theta}$. Linear BLs without intercept only make sense if covariates are centered (see \citebutton{\texttt{mboost}}{https://cran.r-project.org/web/packages/mboost/vignettes/mboost_tutorial.pdf} tutorial, p.~7)}
  \end{itemize}
