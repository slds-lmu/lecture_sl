...
\begin{itemize}
    \item 
        If category $g^\ast\in\{1, \dots, G\}$ is not informative, the respective parameter estimate $\thetah_{g^\ast}$ is (almost sure) not $0$.
\end{itemize}
...
\begin{itemize}
    \item 
        Fitting CWB is much slower due to the overhead in each iteration %(updating the model, pseudo residuals, and doing the base learner selection) 
        after conducting one tiny step into the direction of the 1-dim parameter $\thetah_{j,g}$.

    \item 
        Base learner has exactly one degree of freedom $\rightarrow$ penalization and a fair selection hard to intractable.
\end{itemize}
...
Consider the \texttt{life expectancy} (data set is provided by the WHO and is available on \citebutton{Kaggle}{https://www.kaggle.com/datasets/kumarajarshi/life-expectancy-who}\,: regression task, for which we fit a
