...
\begin{itemize}
    ...
    \item The number $\sum_{m=1}^M \mathds{1}_{[j = j^{[m]}]}$ tells us how often the feature $x_j$ in $b_j$ were selected
\end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item 
        ...
        Including all classes of the feature means adding $G$ base learners $b_{j,1}, \dots, b_{j,G}$ with each accounting for one specific class.
  \end{itemize}
...
\begin{itemize}
  \item The loss-optimal constant $\fm[0](\xv)$ is a initial model intercept.
