...
However, a unique stationary point must exist since $z_j$ is strictly convex for $H_{j,j} > 0$. This means the only possible minimizer of $z_j$ is $\hat{\theta}_{\text{lasso},j} = 0$. %This means, here, $z_j$ is strictly monotonically decreasing (increasing) for $\theta_j < 0 $ ($\theta_j > 0 $). 
\\
\end{minipage}
 \\
