...
\titlemeta{% Chunk title (example: CART, Forests, Boosting, ...), can be empty
  ...
  \item Know the properties of ridge vs. lasso regularization 
  \item Understand the effects of the methods on model coefficients
  \item Understand the importance of standardization in regularization due to lack of rescaling equivariance
}
...
$$ 
  ...
  \begin{itemize}
    ...
    \item In the $p>n$ case, lasso selects at most $n$ features \citelink{ZOUHASTIE}.
...
\begin{center}
\includegraphics[width=0.7\textwidth]{figure/regu_example_multicollinearity.png}
\end{center}
...
Ridge has the so-called grouping property: highly corr. features tend to have equal effects. Lasso however ``decides`` on one of the features.
\vspace{0.15cm}

But why did lasso select $x_4$ oder $x_5$ ?\\ \textbf{Short answer}: highly corr. features are selected ``randomly''/arbitrarily.

\framebreak

The ``random'' decision is in fact a complex deterministic interaction of data geometry (e.g., corr. structures), the optimization method, and its hyperparamters (e.g., initialization). The theoretical reason for this behavior relates to the convexity of the penalties \citelink{ZOUHASTIE}.

\vspace{0.2cm}

For extreme cases, we can obtain some instructive formal intuition for this phenomenon. Consider perfectly collinear features $x_4=x_5$:
\vspace{0.15cm}
\begin{itemize}
    ...
    \item $L1$ penalty is not \textit{strictly} convex. Hence no unique solution exists if $x_4=x_5$, and sum of both coefficients can be allocated arbitrarily to both features while staying minimizers (no grouping property!):\\
    For any solution $\thetah_{4,lasso},\thetah_{5,lasso}$, equivalent minimizers are given by 
    {\small $$\Tilde{\theta}_{4,lasso}=s\cdot(\thetah_{4,lasso}+\thetah_{5,lasso}) \,\,\text{and}\,\,\Tilde{\theta}_{4,lasso}=(1-s)\cdot(\thetah_{4,lasso}+\thetah_{5,lasso})\,\forall s\in[0,1]$$}
\end{itemize}
...
\begin{vbframe}{Synopsis \citelink{TIBS1996} \citelink{ZOUHASTIE}}
...
\item For lasso one of the correlated predictors will have a larger coefficient, while the rest are (nearly) zeroed. The respective feature is, however, selected ``arbitrarly''
