...
\titlemeta{% Chunk title (example: CART, Forests, Boosting, ...), can be empty
  ...
  \item What happens under correlated features
...
$$ 
  ...
  \begin{itemize}
    ...
    \item For lasso, solution likely touches a corner  constraint region. This induces sparsity and is a form of variable selection.
...
\begin{itemize}
    \item For the lasso, this also implies that for very correlated features, we could arbitrarily force a feature out of the model through a unit change.
\end{itemize}
...
But why did lasso select $x_4$ over $x_5$ ?\\ \textbf{Short answer}: highly corr. features are selected ``randomly''/arbitrarily
...
\begin{itemize}
    ...
    \item $L1$ penalty is not \textit{strictly} convex. Hence no unique solution exists if $x_4=x_5$, and sum of coefficients can be allocated arbitrarily to both features while remaining minimizers (no grouping property!):\\
