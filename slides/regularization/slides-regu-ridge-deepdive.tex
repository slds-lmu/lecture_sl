...
For linear model $\yv = \Xmat \thetab + \bm{\varepsilon}$ with fixed $\Xmat \in \mathbb{R}^{n \times p}\,\text{and}\,\bm{\varepsilon} \sim (\bm{0},\sigma^2 \bm{I}_n)$, bias of ridge estimator $\thetah_{\text{ridge}}$ is given by 
\begin{equation*}
    \begin{aligned}
...
%Assume that our targets are generated by $y=f(\xv)+\varepsilon$ where $\varepsilon \sim (0,\sigma^2)$ and $f(\xv)=\thetab^{\top}\xv$.
\vspace{0.2cm}

The bias/variance of predictions $\hat{f}:=\fh(\xv)=\thetah_{\text{ridge}}^{\top} \xv$ can be obtained as:
...
We can now decompose the MSE of $\fh$ given a fresh sample $(y,\xv)$ as

$$\text{MSE}(\fh)=\E[(y-\fh(\xv))^2]=\text{Bias}^{2}(\fh) + \text{Var}(\fh) + \sigma^2$$

This decomposition is similar to the statistical inference setting before, however, the irreducible error $\sigma^2$ only appears for predictions, as it is an artifact
