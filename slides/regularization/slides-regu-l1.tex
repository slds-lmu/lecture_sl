...
Let $\Xmat_1$ denote the $n \times q$ matrix with the relevant features and $\Xmat_2$ the matrix containing noise features. It can be proven that $\thetah_{lasso}$ is sign consistent under a \textbf{irrepresentable condition} \citebutton{Zhao and Yu, 2006}{https://www.jmlr.org/papers/volume7/zhao06a/zhao06a.pdf}:
\begin{align*}
    \vert (\Xmat_2^{\top} \Xmat_1^{\top})^{-1} (\Xmat_1^{\top} \Xmat_1) \text{sign}(\thetab_1)\vert < \bm{1} %\,\, (\text{element-wise})
\end{align*}
