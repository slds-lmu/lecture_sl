...
$$
\thetav ~|~ \Xmat, \yv \sim \normal(\sigma^{-2} \Kmatinv\Xmat^T\yv, \Kmatinv)
$$
...
&\Leftrightarrow & \sigma^{-2} \yv^T \Xmat \Kmatinv = c^T \\
&\Leftrightarrow& c = \sigma^{-2} \Kmatinv \Xmat^T \yv
\end{eqnarray*}
\item Finally: $\thetav ~|~ \Xmat, \yv \sim  \normal(\sigma^{-2}\Kmatinv\Xmat^T\yv, \Kmatinv) \qed$
\end{framei}
...
\item For a new observation $\xstar$ we get
$$
y_* ~|~ \Xmat, \yv, \xstar \sim \normal(\sigma^{-2}\yv^T \Xmat \Kmatinv\xstar, \xstar^T\Kmatinv\xstar*)
$$
...
\item For every test input $\xstar$, we get a posterior mean (orange) \& variance (grey region; $\pm 2\times$ standard deviation)
\vfill
...
\item From posterior distribution of $\thetav$ we can derive a predictive distribution for $y_* = \thetav^T \xstar$
