...
\begin{eqnarray*}
    ...
    &\sim& \normal\left(0.61, 0.6279\right) 
  \end{eqnarray*}
\item MAP-estimate: $f(\xv_*) = 0.61$, uncertainty estimate: $0.6279$
\end{framei}
...
$$
  \fv_* = \left[f\left(\xi[1]_*\right), ..., f\left(\xi[m]_*\right)\right]. 
$$
  ...
  $$
    ...
    \fv_*
    \end{bmatrix} \sim  
    \normal\biggl(\bm{0}, \begin{bmatrix} \Kmat & \Kmat_* \\ \Kmat_*^T & \Kmat_{**} \end{bmatrix}\biggr),
  $$
...
\begin{eqnarray*}
\fv ~|~ \Xmat, \fv \sim \normal(\Kmat\Kmat^{-1}\fv, \Kmat - \Kmat^T \Kmat^{-1} \Kmat) = \normal(\fv, \bm{0}).
\end{eqnarray*}
