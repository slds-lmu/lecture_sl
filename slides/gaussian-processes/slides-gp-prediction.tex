...
\begin{itemize}
\item $\bm{z} \sim \normal(\muv, \Sigma)$, partition $\bm{z} = (\bm{z}_1^T, \bm{z}_2^T)^T$ s.t. $\bm{z}_1 \in \R^{m_1}, \bm{z}_2 \in \R^{m_2}$, $$\muv = (\muv_1^T, \muv_2^T)^T, \quad \Sigma = \begin{bmatrix} \Sigma_{11} & \Sigma_{12} \\ \Sigma_{21} & \Sigma_{22} \end{bmatrix} $$
...
\vfill
\imageC[.8]{figure/gp_pred/gp_interpolator_distrib.pdf}
\end{framei}
