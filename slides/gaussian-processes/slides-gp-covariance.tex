...
\vfill 
\splitV{ \imageC[.9]{figure/cov_funs/example_covariance_1.pdf} }{ \imageC[.9]{figure/cov_funs/example_function_1_1.pdf} } 
\end{framei} 
...
\vfill 
\splitV{ \imageC[.9]{figure/cov_funs/example_covariance_1.pdf} }{ \imageC[.9]{figure/cov_funs/example_function_1_2.pdf} } 
\end{framei} 
...
\vfill 
\splitV{ \imageC[.9]{figure/cov_funs/example_covariance_2.pdf} }{ \imageC[.9]{figure/cov_funs/example_function_2_1.pdf} } 
\end{framei}


\begin{framei}[sep=M]{properties of covariance functions I}
...
\imageC[0.9]{figure/stationary_vs_nonstationary.pdf}
\item Stationarity implies: contour set of $\xv \to k(\xv,\xtil_1)$ must be same as for $\xv \to k(\xv, \xtil_2)$ translated by $\xtil_1-\xtil_2$ 
\item Left: contours are parallel lines to diagonal $\checkmark$\\ 
Right: non-lin. contours, not translation-invariant $\Rightarrow$ non-stationary
\end{framei}

\begin{framei}[sep=L]{properties of covariance functions II}
%\item Most cov. functions have 3 common properties
\item \textbf{Isotropic}: $k = k(r)$ with $r = \xxtnorm$ \\
$\Rightarrow$ invariant to rotations, implies stationarity\\
(again slight notational abuse)
\item Since $k$ is stationary can write $k(\bm{d})$ and plot contours for $\xv, \xtil \in \mathbb{R}^2$
\imageC[0.9]{figure/isotropic_vs_anisotropic.pdf}
\item Isotropic = circle/hypersphere contours\\
Anisotropic = non-spherical ellipsoid contours
...
\item LHS: Random draws. RHS: covariance matrix of 25 sampled points (black = high pos cor, white = high neg cor)
\vfill
...
\item Squared exponential kernel ($\nu = \infty$) and exponential kernel ($\nu = 0.5$) are special cases
\vfill
...
\item In SVMs we called this bandwith or (inverse) kernel width
\end{framei}
