...
$\Rightarrow$ proof: \furtherreading{GRIMMETT2001PROC} (Thm. 8.6.3)
\end{framei}
...
\begin{eqnarray*}
m(\xv) &=& \E[f(\xv)] \\
\kxxt &=& \cov(\fx, f(\xtil)) = \E \left[\left( f(\xv) - \E[f(\xv)] \right) \left( f(\xtil) - \E[f(\xtil)] \right) \right]
\end{eqnarray*}
\item For now, we consider zero-mean GPs with $m(\xv) \equiv \zero$ 

$\Rightarrow$ common, not necessarily drastic assumption 
\item Denote by $\gpzk$ $\Rightarrow$ properties mainly governed by $k(\cdot, \cdot)$
