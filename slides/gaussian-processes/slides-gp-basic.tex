...
figure_man/discrete/marginalization-more3.png
}{
...
\begin{framev}{Weight-space vs. Function-space View}
\splitVTT{
...
$$\fv = \fvec^T \sim \Nmk$$
\item For now: assume $\mv, \Kmat$ to be given
\end{framei}
...
\item So far: Multivar Gaussians to model outputs of discrete functions
$$\fv = \fvec^T \sim \Nmk$$
...
\item This mapping is the formal definition of notation $\{s_t: t \in T\}$
...
% \else
\imageC[.8]{figure_man/discrete/marginalization-more3.png}
% \fi
}

\item Example with 1D (left) and 2D (right) index set $\Xspace$: Dimension of $\fv$ depends on $n$, not on dimension of $\Xspace$:
\splitV{
\imageC{figure_man/indexed_family/indexed_family_53.png}
}{
\imageC[.8]{figure_man/indexed_family/indexed_family_63.png}
}
\vfill
...
will talk more about this in session on GPs with trend
