...
plotBivariateDensity <- function(xlim, ylim, mu, sigma) {
	...
	grid <- expand.grid(y1 = seq(xlim[1], xlim[2], length.out = 100),
						y2 = seq(ylim[1], ylim[2], length.out = 100))

	probs <- cbind(grid,
				   density = mvtnorm::dmvnorm(grid, mean = mu, sigma = sigma))

	p2 <- ggplot() +
		xlab(expression(y[1])) +
		ylab(expression(y[2])) +
		geom_raster(data = probs,
					 aes(x = y1, y = y2, fill = density)) +
		geom_contour(data = probs,
					 aes(x = y1, y = y2, z = density),
					 colour = "white",
					 bins = 5) +
...
createMat <- function(n, scaleFactor) {
  ...
  if (scaleFactor< 0 | scaleFactor > 1)
	  stop("Scaling factor must be between 0 and 1")
...
for (ninputs in c(2, 5, 10)) {
	...
	for (i in 1:3) {
		...
		ggsave(filename = paste0("../figure/discrete/example_", ninputs, "_", i, ".pdf"), plot = p, height = 2.5, width = 4)
	}
}
...
for (input in ninputs) {
	...
	for (i in 1:3) {
		...
		p1 <- plotDiscreteFunction(x, y, xlim = c(0, 1), ylim = c(- 2, 3)) +
			ggtitle(paste0("Sample Function ", i, ", n = ", input))

		if (input == 2) {
			p2 <- plotBivariateDensity(mu = muc,
									   sigma = sigmac,
									   xlim = c(-2, 3),
									   ylim = c(-2, 3)) +
				geom_point(data = data.frame(),
						   ...
						   size = 3) +
				theme_bw() +
				ggtitle("Density of a 2-D Gaussian") +
				ylab(expression(h[2])) +
				xlab(expression(h[1]))
		}
		...
		ggsave(paste0("../figure/discrete/example_norm_", input, "_", i, "_a.pdf"), p1, width = 3, height = 3)
		ggsave(paste0("../figure/discrete/example_norm_", input, "_", i, "_b.pdf"), p2, width = 3, height = 3)
...
plist <- lapply(ninputs, function(input) {
	...
	p <- plotDiscreteFunction(x, y, xlim = c(0, 4), ylim = c(- 2, 2)) +
		ggtitle(paste0("n = ", input)) +
		theme(axis.text.x = element_blank())
	p
})

ggsave(paste0("../figure/discrete/example_limit.pdf"), do.call(grid.arrange, c(plist, nrow = 1)), width = 8, height = 3)
...
for (input in ninputs) {
	...
	p1 <- plotDiscreteFunction(x, y, xlim = c(0, 4), ylim = c(- 2, 2)) +
		ggtitle(paste0("Sample Function, n = ", input)) +
		theme(axis.text.x = element_blank())

	ggsave(paste0("../figure/discrete/example_marginalization_", input, ".pdf"), p1, width = 3, height = 3)
}
