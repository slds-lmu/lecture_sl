...
\item[] 
\begin{equation}
\partial D_{KL}(f||q) / \partial \mu = \partial / \partial \mu (- \mathbb{E}_f \log [q(X|\theta)]) = \mathbb{E}_f \frac{1}{\sigma^2} (X-\mu) 
\label{eq:I}
\end{equation}
\item[] 
\begin{equation}
\partial D_{KL}(f||q) / \partial \sigma^2 = \partial / \partial \sigma^2(- \mathbb{E}_f \log [q(X|\theta)]) = \mathbb{E}_f [\frac{1}{2\sigma^2} + \frac{-1}{2\sigma^4} (X-\mu)^2] 
\label{eq:II}
\end{equation}
\end{itemize}
