\begin{enumerate}
  ...
  \item 
    ...
    \begin{align*}
      H(y|x) &= \E_x [H(y|x=x)] = \sum_{x \in \Omega_x } H(y|x=x) \\
  ...
  \begin{itemize}
    \item
      As $\epsilon$ and $x$ are have a deterministic relation, the uncertainty introduced by the $\psi$ function is 0. The entropy of $\epsilon$ is given only by the entropy of $x$
      \begin{align*}
        H(\epsilon) = H(x)
      \end{align*}
