...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item and for $f(\theta) =  \lambda\vert \theta \vert$ and $\lambda > 0,$ it holds that $\partial f(\lambda) = \begin{cases} \{-\lambda\} & \text{ for }\lambda < 0\\
        [-\lambda, \lambda] & \text{ for }\lambda = 0 \\
