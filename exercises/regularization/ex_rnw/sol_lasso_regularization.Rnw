\begin{enumerate}
    ...
    \item 
        ...
        First of all, we will use the  fact that $\Xmat$ has orthonormal column to show that:
        ...
        \begin{equation}
            \begin{aligned}
                \argmin_{\thetab}  \riskrt &= \argmin_{\thetab} \frac12 \sum_{i=1}^n \left( \yi - \thetab^\top \xi \right)^2 + \lambda \sum_{i=1}^p  |\theta_i| \\
                &= \argmin_{\thetab}  \frac12  \| \yv - \xv \thetab  \|_2^2 + \lambda \|\thetab\|_1 \\
                &= \argmin_{\thetab} \frac12 ( \yv - \xv \thetab)^T ( \yv - \xv \thetab) + \lambda \|\thetab\|_1 \\ 
                &= \argmin_{\thetab}  \frac12 \left(\underbrace{\yv^T \yv}_{indep. \  of \  \thetab} - 2 \underbrace{\yv^T \xv}_{\thetab^T} \theta + \theta^T \underbrace{\xv^T \xv}_{\id} \thetab \right) + \lambda \|\thetab\|_1 \\
    ...
    \item
        ...
        The advantage of this representation is that we can optimize each $g_i(\theta)$ separately to get optimal entries for $\theta$.
    ...
    \item 
        ...
        \begin{equation} \label{eq:compare_gi}
            \begin{aligned}
                g_i(\theta) &= \underbrace{- \thetah_{i}\theta} + \frac{\theta^2}{2} + \lambda|\theta_i| \\
        ...
        \begin{equation}
            \begin{aligned}
                - \thetah_{i}\theta < \thetah_{i}\theta \longrightarrow \theta_i^* \geq 0
            \end{aligned}
        \end{equation}
    ...
    \item 
        ...
        \begin{equation}
            \begin{aligned}
                ...
                \theta_i^* &= max (0, \underbrace{\thetah_i - \lambda}_{>0}) \\
    ...
    \item 
        ...
        The second and third term in both equations are equivalent. Using the condition from eq. \ref{eq:compare_gi_2} again, we can conclude that the minimizer $\theta_i^*$ must be non-positive.

        \begin{equation}
            \begin{aligned}
                - \thetah_{i}\theta < \thetah_{i}\theta \longrightarrow \theta_i^* \leq 0
            \end{aligned}
        \end{equation}
    ...
    \item 
        ...
        \begin{equation} \label{eq:minimizer_theta_less_0}
            \begin{aligned}
                ...
                \theta_i^* &= 0, \underbrace{\thetah_i + \lambda}_{\leq0} \\
    ...
    \item 
        ...
        By transforming the min function from \ref{eq:minimizer_theta_less_0} to a max function, combine the two cases in only one equation:
