...
where $\thetabh = (\thetah_{1},\ldots,\thetah_{p})^\top = ({\Xmat}^T \Xmat)^{-1} \Xmat^T\yv$ is the minimizer of the unregularized empirical risk (w.r.t.\ the L2 loss). For this purpose, use the following steps:
\begin{itemize}	
	\item
		Derive that 
	...
	\item 
		...
		Note that the minimization problem on the right-hand side of (i) can be written as $\sum_{i=1}^p g_i(\theta_i),$ where 
		$$ 
	...
	\item 
		Derive that $\theta^*_i = \max\{\thetah_{i} - \lambda,0 \},$   by using (iii) (and also still considering the case $\thetah_i > 0.$)
	...
	\item 
		...
		Consider the complementary case of (iii) and (iv), i.e., $\thetah_i \leq 0,$ and infer that for the minimizer $\theta^*_i$ of $g_i$ it must hold that $\theta^*_i \leq 0.$
	...
	\item 
		...
		Derive that $\theta^*_i = \min\{ 1\thetah_{i} + \lambda,0 \},$   by using (v) (and also still considering the case $\thetah_i \leq 0.$)
