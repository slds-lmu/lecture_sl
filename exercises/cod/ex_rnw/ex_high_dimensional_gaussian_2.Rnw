...
\begin{enumerate}
    \item 
        What is $\E[\|X\|_1]?$ (Hint: $\E|X|=\sqrt{\frac{2}{\pi}}.$)

    \item 
        What is $\var[\|X\|_1]?$ (Hint: $\var|X|=1-\frac{2}{\pi}.$)
    \item 
    ...
    \item 
        What is $\var (  \| X - X' \|_1)?$
    \item
        Let $\xv=(x_1,\ldots,x_p)^\top \in \R^p$  be some arbitrary deterministic vector. Compute $\E\langle X,\xv \rangle$ and $\var(\langle X,\xv \rangle).$
\end{enumerate}